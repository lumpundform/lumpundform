	...
	public void teletransportar(Vector2 pos, Poligono piso) {
		if (colisionPiso) {
			...
			if (piso.estaColisionando(getHitbox())) {
				...
				Linea lii = piso.estaColisionando(infIzq) ? piso.linea("arriba", infIzq) : null;
				Linea lid = piso.estaColisionando(infDer) ? piso.linea("arriba", infDer) : null;
				Linea lsi = piso.estaColisionando(supIzq) ? piso.linea("abajo", supIzq) : null;
				Linea lsd = piso.estaColisionando(supDer) ? piso.linea("abajo", supDer) : null;
				Float yii = null, yid = null, ysi = null, ysd = null;
				Float yArribaIzq = null, yArribaDer = null, yAbajoIzq = null,
						yAbajoDer = null, yArriba = null, yAbajo = null;
				
				if (lii != null || lid != null) {
					if (lii != null) { yii = lii.yEnX(infIzq); }
					if (lid != null) { yid = lid.yEnX(infDer); }
					if (yii != null) { yArribaIzq = yii - infIzq.y; }
					if (yid != null) { yArribaDer = yid - infDer.y; }
					
					if (yArribaIzq == null || yArribaDer == null) {
						yArriba = yArribaIzq != null ? yArribaIzq : yArribaDer;
					} else {
						yArriba = yArribaIzq > yArribaDer ? yArribaIzq : yArribaDer;
					}
				}
				if (lsi != null || lsd != null) {
					if (lsi != null) { ysi = lsi.yEnX(supIzq); }
					if (lsd != null) { ysd = lsd.yEnX(supDer); }
					if (ysi != null) { yAbajoIzq = supIzq.y - ysi; }
					if (ysd != null) { yAbajoDer = supDer.y - ysd; }
				...
				if (yArriba != null || yAbajo != null) {
					...
					if (yArriba != null && yAbajo != null) {
						...
						float yFinalIzq = piso.linea("arriba", supIzq).yEnX(supIzq);
						float yFinalDer = piso.linea("arriba", supDer).yEnX(supDer);
